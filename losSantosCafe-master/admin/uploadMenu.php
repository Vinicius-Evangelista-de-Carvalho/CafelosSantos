...
$target_dir = "../img/menu/".$_POST["tipoBebida"]."/";
...
if ($uploadOk == 0) {
$location="Location: menus/agregarMenu.php?error=".$error;
// Sí no existe ningún error, entonces se procede a subir la imagen y a registrarla en la base
    ...
    if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
        ...
        $idTipoBebida = $con->getTipoBebida($tipoBebida);
        
        $ingredientes = $_POST["ingredientes"];
        $nombre=$_POST["nombre"];
        $ruta=$target_file;
        $result= $con->addBebida ($nombre, $ruta, $idTipoBebida, $ingredientes);
        ...
        if (count($result) > 0) {
            ...
            $location="Location: menus/agregarMenu.php?mensaje=".$mensaje;
        } else {
